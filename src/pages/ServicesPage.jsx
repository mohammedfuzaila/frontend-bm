...
const ServicesPage = ({ onOpenAuth, onBookAction, onBookingCreated }) => {
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', position: 'relative' }}>
      ...
      {/* Header */}
      <section className="section" style={{ paddingTop: '5rem', paddingBottom: '3rem', background: 'linear-gradient(180deg, var(--surface2) 0%, transparent 100%)' }}>
        <div className="page-wrapper" style={{ textAlign: 'center' }}>
          ...
          <motion.h1 className="h1" {...si(0.1)} style={{ fontWeight: 950, letterSpacing: '-0.05em' }}>
      ...
      <div className="page-wrapper" style={{ paddingBottom: '8rem', position: 'relative', zIndex: 1 }}>
        {loading ? (
          <div style={{ display: 'grid', gap: '4rem' }}>
            {[1, 2].map(i => (
              <div key={i}>
                <div style={{ width: 220, height: 32, background: 'white', borderRadius: 12, marginBottom: '1.5rem', animation: 'pulse 1.5s infinite' }} />
            ...
            return (
              <section key={cat.id} style={{ marginBottom: '6rem' }}>
                <motion.div 
                  ...
                  <div style={{ width: 64, height: 64, borderRadius: '20px', background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 12px 24px rgba(92,98,241,0.1)', border: '1px solid var(--border)', color: 'var(--primary)', flexShrink: 0 }}>
                    {categoryIcons[cat.name] || <Grid size={28} />}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <h2 style={{ fontWeight: 950, fontSize: '2rem', letterSpacing: '-0.04em', margin: 0, color: 'var(--dark)' }}>{cat.name}</h2>
                    <p style={{ color: 'var(--primary)', fontSize: '0.95rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{services.length} Premium Options</p>
                  </div>
                  <div style={{ flex: 1, height: 2, background: 'var(--border)', marginLeft: '1.5rem', opacity: 0.3, borderRadius: 2 }} />
                </motion.div>
                  ...
                  {services.map((s, idx) => (
                    ...
                    >
                      <div style={{ height: 230, overflow: 'hidden', position: 'relative' }}>
                        ...
                        />
                        <div style={{ position: 'absolute', top: '0.75rem', right: '0.75rem', background: 'rgba(255,255,255,0.95)', padding: '0.5rem 1rem', borderRadius: '12px', fontWeight: 950, fontSize: '1rem', color: 'var(--dark)', backdropFilter: 'blur(10px)', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', zIndex: 2 }}>
                          ₹{s.price}
                        </div>
                      </div>
                      <div style={{ padding: '1.5rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                        <h3 className="h3" style={{ marginBottom: '0.75rem' }}>{s.title}</h3>
                        <p style={{ color: 'var(--muted)', fontSize: '0.95rem', lineHeight: 1.6, marginBottom: '2.5rem', flex: 1 }}>{s.description}</p>
                        
                        <motion.button
                          className="btn btn-primary glow"
                          style={{ width: '100%', borderRadius: '16px', padding: '1.2rem' }}
